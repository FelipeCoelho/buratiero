...
public class CustomPagingNavigator<T, S> extends AbstractToolbar
{
   ...
   public CustomPagingNavigator(final DataTable<T, S> dataView,
      final IPagingLabelProvider labelProvider, final List<Integer> itemsPerPageValues)
   {
      super(dataView);
   ...
   {
      final ListView<Integer> itemsPerPageList =
         new ListView<Integer>("itemsPerPage", itemsPerPageValues)
         {
            ...
            protected void populateItem(final ListItem<Integer> item)
            {
               final Link<T> itemPerPageLink =
                  new ItemPerPageLink<T, S>("itemPerPageLink", dataView, pagingLinksContainer,
                     item.getModelObject());
