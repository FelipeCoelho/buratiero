	...
	private void addLinksChangingItemsPerPageNumber() {
		final ListView<Integer> itemsPerPageList = new ListView<Integer>("itemsPerPage", itemsPerPageValues) {
			...
			protected void populateItem(final ListItem<Integer> item) {
				final Link<T> itemPerPageLink = new ItemPerPageLink<T, S>("itemPerPageLink", dataView, customPagingNavigator, item.getModelObject());
				itemPerPageLink.add(new Label("itemsValue", item.getModel()));
				itemPerPageLink.add(AttributeModifier.replace("class", "btn " + ((Long.valueOf(item.getModelObject()) == dataView.getItemsPerPage()) ? "btn-primary" : "btn-default")));
